      ...
      <Table className="table-report text-sm">
        <thead >
          <tr>
            ...
            <th colSpan={2}>
              Кол-во вакцинированных, подвергнутых диагностическим
              исследованиям животных (тыс. гол.)
            </th>
          ...
          <tr>
            {rowNums.map((item, index)=> <th key={index}>{item}</th>)}
          </tr>
          
        </thead>
        <tbody>
          {data.map(drug => <Vet1BTableItem key={drug.id} drug={drug} />)}
        </tbody>
      </Table>
    </Container>
  );
