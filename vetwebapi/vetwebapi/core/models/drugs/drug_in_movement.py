...
class DrugInMovement(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "drug_movement_id", "catalog_drug_id", name="idx_unique_drug_in_movement"
        ),
    )
    ...
    drug_movement: Mapped["DrugMovement"] = relationship(
        back_populates="catalog_drugs_details"
    )
    catalog_drug: Mapped["CatalogDrug"] = relationship(
        back_populates="drug_movements_details"
    )
