...
async def create_vetwork(session: AsyncSession, body: VetWorkIn) -> None: 
    diseases = body.diseases
    doctors = body.doctors
    new_vetwork = VetWork(**body.model_dump(exclude=[body.diseases, body.doctors]))
...
async def add_doctors_in_vetwork(session: AsyncSession, vetwork_id: int, doctors: list[int]) -> None:
    new_relations = [DoctorInVetWork(vetwork_id=vetwork_id, doctor_id=item) for item in doctors]
    session.add_all(new_relations)
    await session.commit()
