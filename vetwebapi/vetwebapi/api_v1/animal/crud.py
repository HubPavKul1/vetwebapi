...
from vetwebapi.core.models import Company, Animal, TypeOfFeeding, Species, Gender, UsageType, AnimalGroup

from .schemas import AnimalIn
...
async def read_company_animals(session: AsyncSession, company_id: int) -> list[Animal | None]:
    stmt = select(Animal).where(and_(Animal.company_id == company_id, Animal.is_active == True)).order_by(Animal.nickname)
